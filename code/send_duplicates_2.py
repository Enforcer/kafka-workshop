...
def main() -> None:
    ...
    message = generate()
    for version in range(1, random.randint(1, 3) + 1):
        message_id = int(time.time())
        new_message = generate()
        message["keywords"] = new_message["keywords"]
        message["availability"] = new_message["availability"]
        for _ in range(random.randint(2, 5)):
            producer.produce(
                topic=TOPIC,
                value=json.dumps({**message, "version": version}),
